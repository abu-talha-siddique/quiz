...
import hbs from "hbs";
// import partials from "express-partials"; // Check if installed, otherwise remove/comment
import cors from "cors";
import jwt from "jsonwebtoken";
import cookieParser from "cookie-parser";
// import bodyParser from "body-parser"; // Express has built-in body parser
import { fileURLToPath } from 'url';

import StudentData from "./models/registers.js";
import { registrationMail } from "./middleware/registrationMail.js";
// import { submissionMail } from "./controller/submissionMail.js"; // Missing file
...
app.post("/uploadPdf", upload.single("file"), async (req, res) => {
    try {
        ...
        const type = "pdf";
        const uploadedPdf = await StudentData.updateOne({ _id: _id }, { $push: { UploadedFiles: { type: type, title: title, pdf: filename, date: date } } });
...
app.post("/deletePdf", async (req, res) => {
    try {
        const _id = "67014558c79b90ed53ef6ec3";
        const uploadedPdf = await StudentData.updateOne({ _id: _id }, { $pull: { UploadedFiles: { pdf: req.body.pdf } } });
...
app.post("/deleteQuiz", async (req, res) => {
    try {
        ...
        if (ifExists) {
            const remove = await StudentData.deleteOne({ _id: ifExists._id });
...
app.post("/addQuestion", async (req, res) => {
    try {
        ...
        if (quizData) {
            const addedQuestion = await StudentData.updateOne({ _id: quizData._id }, { $push: { Query: { question: req.body.question, opt1: req.body.opt1, opt2: req.body.opt2, opt3: req.body.opt3, opt4: req.body.opt4, answer: req.body.answer } } });
...
app.post("/deleteQuestion", async (req, res) => {
    try {
        ...
        const ifExists = await StudentData.findOne({ type: type, quizName: quizName });
        if (ifExists) {
            const remove = await StudentData.updateOne({ _id: ifExists._id }, { $pull: { Query: { question: req.body.questionName } } });
...
app.post("/particularQuizDetail", async (req, res) => {
    try {
        ...
        const quizInfoDetail = await StudentData.findOne({ type: type, quizName: quizInfo })
            .then(values => {
...
app.post("/savedAnswer", async (req, res) => {
    try {
        ...
        if (student) {
            ...
            if (changeAnswer) {
                const remove = await StudentData.updateOne({ _id: student._id }, { $pull: { Responses: { questionId: req.body.currentQuestion } } });
                const add = await StudentData.updateOne({ _id: student._id }, { $push: { Responses: { questionId: req.body.currentQuestion, choosen: req.body.clickedOption, rightAnswer: rightAnswer } } });
                res.json({ status: "answer changed" });
            }
            else {
                const savedAnswer = await StudentData.updateOne({ _id: student._id }, { $push: { Responses: { questionId: req.body.currentQuestion, choosen: req.body.clickedOption, rightAnswer: rightAnswer } } });
...
app.post("/clearAnswer", async (req, res) => {
    try {
        ...
        const student = await StudentData.findOne({ quizId: search });
        if (student) {
            const clearAnswers = await StudentData.updateMany({ _id: student._id }, { $pull: { Responses: { questionId: req.body.currentQuestion } } });
...
app.post("/result", async (req, res) => {
    try {
        ...
        const email = req.body.email;
        const search = quizName + "-" + email;
        await StudentData.findOne({ type: type, quizId: search })
            .then(values => {
                res.json(values);
            })
            .catch((error) => {
...
app.post("/confirmation", async (req, res) => {
    try {
        const quizName = req.body.quizInfo;
        const email = req.body.email;
        // submissionMail(email, quizName); // Disabled: Missing controller
        res.json("mail sent");
    }
    catch (error) {
        console.log(error);
...
app.post("/score", async (req, res) => {
    try {
        ...
        if (student) {
            ...
            const saveScore = await StudentData.updateMany({ _id: student._id }, { correct: req.body.correct, wrong: req.body.wrong, score: score });
            res.json({ status: "ok" });
        }
    }
    catch (error) {
